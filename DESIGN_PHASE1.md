...
- ライフが0以下になった通知のみを提供
- ゲーム終了判定は層3が複数の敗北条件と組み合わせて実行

**【重要な注記】**

ゲン＝ガタではライフを整数値として簡略管理します。

しかし本来の Battle Spirits では、ライフはライフゾーン上のコアです。

正式実装時には、LifeRule は CoreRule および ZoneStateRule と統合される予定です。
...
本来の Battle Spirits（正式実装）:
  ├─ CoreRule: ゲーム全体のコア管理
  ├─ ZoneStateRule: ライフゾーンの管理
  └─ LifeRule は CoreRule/ZoneStateRule に統合
      （ライフゾーン上のコア数 = ライフ）
```

**現在の層設計との整合性:**

ゲン＝ガタ段階では LifeRule を独立したルールとして扱うことで：
- ルール構造を明確に保つ
- 将来の複雑化に対応しやすくする
- テスト・保守性を向上させる

この設計は「正式実装への段階的移行」を視野に入れています。
