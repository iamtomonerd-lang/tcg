...
export function renderState(state: GameState): string {
  ...
  lines.push(`${mark0} P0  Life: ${String(p0.life).padStart(2)} | Spirits: ${p0.spirits.length} | Nexuses: ${p0.nexuses.length} | Hand: ${p0.hand.length}`);
  ...
  lines.push(`${mark1} P1  Life: ${String(p1.life).padStart(2)} | Spirits: ${p1.spirits.length} | Nexuses: ${p1.nexuses.length} | Hand: ${p1.hand.length}`);
